    ...
    try {
      ...
      if (!error && data && data.length > 0) {
        const codes: PromoCode[] = data.map((item: Record<string, unknown>) => ({
          ...
          category: (item.category as 'free_days' | 'discount' | 'combined') || (item.discount_percent && Number(item.discount_percent) > 0 ? (Number(item.duration_days) > 0 ? 'combined' : 'discount') : 'free_days'),
          durationDays: Number(item.duration_days || 0),
          maxUses: Number(item.max_uses || 50),
          isUnlimited: Boolean(item.is_unlimited || Number(item.max_uses) >= 999999),
  ...
  try {
    ...
    for (const pc of codes) {
      const { error } = await supabase.from('promo_codes').upsert({
        id: pc.id,
        code: pc.code,
        category: pc.category || (pc.discountPercent ? (pc.durationDays ? 'combined' : 'discount') : 'free_days'),
        duration_days: pc.durationDays || 0,
        max_uses: pc.isUnlimited ? 999999 : (pc.maxUses || 50),
        is_unlimited: Boolean(pc.isUnlimited),
        used_count: pc.usedCount || 0,
