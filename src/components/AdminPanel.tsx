                ...
                <div className="space-y-3 p-4 bg-slate-900/60 rounded-2xl border border-slate-800">
                  <div className="flex items-center justify-between">
                    <label className="block text-xs font-extrabold text-indigo-600 dark:text-indigo-400">
                      📻 Audio MP3 URL (Supabase Storage / Öffentliche MP3 URL):
                    </label>
                    <span className="text-[10px] text-slate-400 font-mono">1 Audio-Datei pro Modul</span>
                  </div>
                  <input
                    type="text"
                    value={vAudioUrl}
                    onChange={(e) => setVAudioUrl(e.target.value)}
                    placeholder="https://<your-supabase-url>/storage/v1/object/public/audio-files/audio.mp3"
                    className="w-full px-3 py-2.5 glass-input rounded-xl text-xs font-mono"
                  />
                  <div className="text-[11px] text-slate-400 flex items-center gap-1.5">
                    <span>💡 <strong>Tipp:</strong> Laden Sie die MP3-Datei in Supabase Storage hoch (z.B. Bucket <code>audio-files</code>) und fügen Sie hier die öffentliche URL ein.</span>
                  </div>
                  {vAudioUrl && (
                    <div className="pt-2 border-t border-slate-800">
                      <span className="block text-[11px] font-extrabold text-indigo-600 dark:text-indigo-400 mb-1.5">
                        ▶️ Live Audio-Player Vorschau (Testen vor dem Speichern):
                      </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-extrabold text-indigo-600 dark:text-indigo-400 mb-1">
                    ...
                    {selectedTileType.startsWith('hoeren') && 'Skript / Transkript (Gesamter Hörtext)'}
